...
功能：
    从Tushare获取每天连板个数晋级的股票数据并保存到MySQL数据库中。
    支持增量更新和指定日期范围更新。

使用方法：
    1. 增量更新（从最后更新日期到今天）：
        python limit_step_update.py

    2. 更新指定开始日期到今天的数据：
        python limit_step_update.py --start_date 2024-01-01

    3. 更新指定日期范围的数据：
        python limit_step_update.py --start_date 2024-01-01 --end_date 2024-01-31

    4. 强制更新指定日期范围的数据（会先删除该范围的旧数据）：
        python limit_step_update.py --start_date 2024-01-01 --end_date 2024-01-31 --force
...
def create_limit_step_table():
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS limit_step (
            ...
            nums INT NOT NULL COMMENT '连板次数',
...
def get_last_trade_date():
    ...
    try:
        ...
        cursor.execute("SELECT MAX(trade_date) FROM limit_step")
...
def update_limit_step_data(start_date=None, end_date=None, force_update=False):
    """更新连板天梯数据
        ...
        try:
            ...
            if force_update:
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "DELETE FROM limit_step WHERE trade_date = %s",
                        (trade_date,)
                    )
                    conn.commit()
...
def main():
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='更新连板天梯数据')
    ...
    # 创建表（如果不存在）
    create_limit_step_table()
    
    # 更新数据
    update_limit_step_data(
