...
功能：
    从Tushare获取同花顺行业资金流向数据并保存到MySQL数据库中。
    支持增量更新和指定日期范围更新。

使用方法：
    1. 增量更新（从最后更新日期到今天）：
        python industry_moneyflow_update.py

    2. 更新指定开始日期到今天的数据：
        python industry_moneyflow_update.py --start_date 2024-01-01

    3. 更新指定日期范围的数据：
        python industry_moneyflow_update.py --start_date 2024-01-01 --end_date 2024-01-31

    4. 强制更新指定日期范围的数据（会先删除该范围的旧数据）：
        python industry_moneyflow_update.py --start_date 2024-01-01 --end_date 2024-01-31 --force
...
def create_industry_moneyflow_table():
    """创建行业资金流向表"""
    conn = None
    cursor = None
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS industry_moneyflow (
            ...
            ts_code VARCHAR(12) NOT NULL COMMENT '板块代码',
            industry VARCHAR(50) NOT NULL COMMENT '板块名称',
            lead_stock VARCHAR(50) COMMENT '领涨股票名称',
            close DECIMAL(18,2) COMMENT '收盘指数',
            pct_change DECIMAL(18,2) COMMENT '指数涨跌幅',
            company_num INT COMMENT '公司数量',
            pct_change_stock DECIMAL(18,2) COMMENT '领涨股涨跌幅',
            close_price DECIMAL(18,2) COMMENT '领涨股最新价',
            net_buy_amount DECIMAL(18,2) COMMENT '流入资金(亿元)',
            net_sell_amount DECIMAL(18,2) COMMENT '流出资金(亿元)',
            net_amount DECIMAL(18,2) COMMENT '净额(亿元)',
            created_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
            updated_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
            UNIQUE KEY uk_code_date (ts_code, trade_date)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='行业资金流向';
        """
        
        cursor.execute(create_table_sql)
        logging.info("成功创建行业资金流向表")
...
def get_last_trade_date():
    ...
    try:
        ...
        cursor.execute("SELECT MAX(trade_date) FROM industry_moneyflow")
...
def update_industry_moneyflow_data(start_date=None, end_date=None, force_update=False):
    """更新行业资金流向数据
        ...
        try:
            ...
            if force_update:
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "DELETE FROM industry_moneyflow WHERE trade_date = %s",
                        (trade_date,)
                    )
                    conn.commit()
...
def main():
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='更新行业资金流向数据')
    ...
    # 创建表（如果不存在）
    create_industry_moneyflow_table()
    
    # 更新数据
    update_industry_moneyflow_data(
